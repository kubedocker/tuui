...
<template>
  ...
  <v-data-iterator
:items="promptStore.promptList" :search="promptStore.search" items-per-page="-1"
    :loading="promptStore.loading" @update:options="promptStore.loadPrompts">
    <template #header>
      <v-toolbar class="px-2" rounded="lg">
        <v-text-field
v-model="promptStore.search" density="comfortable" placeholder="Search"
          prepend-inner-icon="mdi-magnify" variant="solo" clearable hide-details></v-text-field>
      </v-toolbar>
    </template>
    <template #default="{ items }">
      <v-container class="pa-2" fluid>
        <v-row dense>
          <v-col v-for="item in items" :key="item.raw.title + ':' + item.raw.name" cols="auto" class="flex-fill">
            <v-card border flat>
              <v-card-item :subtitle="item.raw.name" class="mb-2" :title="item.raw.title">
                <template #append>
                  <v-btn
icon="mdi-lead-pencil" size="small" text="Read" border flat
...
prepend-icon="mdi-account-cog-outline"
      ...
      <v-card-text>
        ...
        <div v-if="promptStore.promptSelect.arguments">
          <br>
          <v-textarea
v-for="argument in promptStore.promptSelect.arguments" :key="argument.name" v-model="argument.content"
            class="mx-2" color="primary" type="text" variant="outlined" :label="argument.name" rows="1"
            auto-grow>
          </v-textarea>
        </div>
        <v-card-actions>
